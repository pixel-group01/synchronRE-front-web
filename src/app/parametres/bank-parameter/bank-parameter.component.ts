...
export class BankParameterComponent implements OnInit {
  ...
  getItems() {
    this.busyGet = this.bankService.getByCriteria((this.currentPage - 1),this.itemsPerPage,(this.itemToSearch.libelle ? this.itemToSearch.libelle : null))
      .subscribe(
        res => {
          if (res && res['content']) {
            this.items = res['content'] as Bank[];
